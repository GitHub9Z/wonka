...
async function seed() {
    try {
        ...
        await UserBuff_1.default.deleteMany({});
        await Box_1.default.deleteMany({});
        await CopyrightFragment_1.default.deleteMany({});
        ...
        const seriesCopyrightCounts = new Map(); // 记录每个系列创建的版权数量
        // 起源系列的版权名称和描述
        ...
        for (let i = 0; i < createdSeries.length; i++) {
            ...
            if (series.name === '起源') {
                ...
                // 为起源系列生成图片URL
                copyrightImages = originNames.map((name, idx) => `https://images.unsplash.com/photo-${1600000000 + idx * 100000}?w=800&h=800&fit=crop`);
            }
