...
exports.joinLotteryPool = joinLotteryPool;
exports.drawLottery = drawLottery;
exports.getUserLotteryRecords = getUserLotteryRecords;
const CopyrightShare_1 = __importDefault(require("../models/CopyrightShare"));
const LotteryPool_1 = __importDefault(require("../models/LotteryPool"));
 ...
 * @param shares 投入的份额数
 */
async function joinLotteryPool(userId, copyrightId, shares) {
    ...
    const existing = await LotteryPool_1.default.findOne({
        userId,
        copyrightId,
        month: currentMonth
    });
    ...
    await share.save();
    // 记录奖池参与
    const lotteryPool = new LotteryPool_1.default({
        userId,
        copyrightId,
        shares,
        month: currentMonth
 ...
 */
async function drawLottery(month) {
    // 获取该月所有参与者
    const participants = await LotteryPool_1.default.find({ month, isWinner: false });
    ...
    for (const winnerId of winners) {
        await LotteryPool_1.default.findByIdAndUpdate(winnerId, { isWinner: true });
    }
    return winners;
}
/**
 ...
 * @returns 参与记录列表
 */
async function getUserLotteryRecords(userId) {
    const records = await LotteryPool_1.default.find({ userId })
        .populate('copyrightId', 'name image')
        .sort({ month: -1 });
    return records.map(r => {
        const copyright = r.copyrightId;
        return {
            id: r._id,
